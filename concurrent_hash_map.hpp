...
class ConcurrentHashMap final
    : public ConcurrentBridge<ConcurrentHashMap<K, V, Hash>> {
private:
    class DelayDispose {
    public:
    ...
    class Node;
    class RecSomeNode
        : public ReclaimBridge /*, public Stock<RecSomeNode, 1000>*/
    {
    public:
        Node* first;
    ...
    public:
        ...
        void reclaim() {
            if (treeBin != nullptr) {
                ...
                while (temp != nullptr) {
                    // assert(dynamic_cast<ForwardingObject*> (temp) ==
                    // nullptr);
                    Node* next = temp->next;
                    delete temp;
                    temp = next;
    ...
    class RecLinkedNode
        : public ReclaimBridge /*, public Stock<RecLinkedNode, 1000>*/
    {
    public:
    ...
    public:
        RecLinkedNode(Node* link_start, Node* link_end)
    ...
    class TreeNode;
    class RecPartialTree
        : public ReclaimBridge /*, public Stock<RecPartialTree, 1000>*/
    {
    public:
        // friend class ConcurrentHashMap;
        TreeBin* tree_bin;
    ...
    public:
        RecPartialTree(TreeBin* tree_bin, TreeNode* lo, TreeNode* hi)
            : tree_bin(tree_bin), lo(lo), hi(hi) {}
    ...
    class BucketTable;
    class RecForwardingTable
        : public ReclaimBridge /*, public Stock<RecForwardingTable, 1000>*/
    {
    public:
        // friend class ConcurrentHashMap;
        BucketTable* table;
    ...
    public:
        ...
        Node(int hash, const K& key, V* val)
            : hash(hash), key(key), val(val), next(nullptr), shallow(true) {}

        Node(int hash, const K& key, V* val, Node* next)
            : hash(hash), key(key), val(val), next(next), shallow(true) {}
    ...
    public:
        BucketTable(int n)
            : tableArray(new std::atomic<Node*>[n]()),
              lock_levels(new std::mutex[n]()),
              length(n),
              share() {}
        ...
        ~BucketTable() {
            for (int i = 0; i < length; ++i) {
                ...
                if (node == nullptr)
                    continue;
                if (ForwardingObject* forwardNode =
                        dynamic_cast<ForwardingObject*>(node)) {
    ...
    public:
        ForwardingObject(BucketTable* nextTable)
            : Node(MOVED, K(), V()), nextTable(nextTable) {
            this->shallow = false;
        }
        ...
        Node* find(int h, const K& k) {
            ...
            for (BucketTable* localTable = nextTable;;) {
                ...
                if ((eh = e->hash) < 0) {
                    if (ForwardingObject* fe =
                            (dynamic_cast<ForwardingObject*>(e))) {
    ...
    public:
        ...
        Node* find(int h, const K& k) {
            for (Node* e = first; e != nullptr;) {
                ...
                if (((s = lockState.load()) & (WAITER | WRITER)) != 0) {
                    ...
                    TreeNode* r;
                    TreeNode* p;
                    p = ((r = root) == nullptr ? nullptr
                                               : r->findTreeNode(h, k));
                    Blocking* w;
                    if (lockState.fetch_add(-READER) == (READER | WAITER) &&
                        (w = waiter.load()) != nullptr) {
                        w->unpark();
                    }
                    return p;
                }
        ...
        void lockRoot(Pin& keepPin) {
            ...
            if (!lockState.compare_exchange_strong(s, WRITER))
                contendedLock(keepPin);  // offload to separate method
        }
            ...
            for (TreeNode* xp;;) {
                if (x == nullptr || x == root)
                    ...
                    if ((xpr = xp->right) != nullptr && xpr->red) {
                        ...
                        xpr = (xp = x->parent) == nullptr ? nullptr
                                                          : xp->right.load();
                    }
                    if (xpr == nullptr)
                        x = xp;
                    ...
                    if (xpl != nullptr && xpl->red) {
                        ...
                        xpl = (xp = x->parent) == nullptr ? nullptr
                                                          : xp->left.load();
                    }
                    if (xpl == nullptr)
                        x = xp;
        ...
        bool flagAndRecTreeNode(bool flag,
                                TreeNode* node,
                                Pin& keepPin,
         ...
         */
        bool removeTreeNode(TreeNode* p,
                            Pin& keepPin,
                            DelayDispose& delayDispose) {
    ...
    static bool casTabAt(std::atomic<Node*>* tab,
                         int i,
                         Node*& old,
                         Node* newNode) {
    ...
    static void setTabAt(std::atomic<Node*>* tab, int i, Node* node) {
        tab[i].store(node);
    }
    ...
    static int resizeStamp(int n) {
        return numberOfLeadingZeros(n) | (1 << (RESIZE_STAMP_BITS - 1));
    }
    ...
    void transfer(BucketTable* localTable, BucketTable* nextTab, Pin& keepPin) {
        ...
        if ((stride = (NCPU > 1) ? (static_cast<unsigned int>(len) >> 3) / NCPU
                                 : len) < MIN_TRANSFER_STRIDE)
        ...
        for (int i = 0, bound = 0;;) {
            ...
            if (i < 0 || i >= len || i + len >= nextn) {
                ...
                if (sizeCtl.compare_exchange_strong(sc, sc - 1)) {
                    if ((sc - 2) != resizeStamp(len) << RESIZE_STAMP_SHIFT)
                        return;
                    finishing = advance = true;
                    i = len;  // recheck and computes how many bytes should to
                              // be reclaimed
                }
                ...
                DelayDispose delayDispose;
                std::lock_guard<std::mutex> control(
                    localTable
                        ->lock_levels[i]);  // std::cout << "KKKK" << std::endl;
                if (tabAt(tab, i) == f) {
                    ...
                    if (fh >= 0) {
                        int runBit = fh & len;
                        Node* lastRun = f;
                        for (Node* p = f->next.load(); p != nullptr;
                             p = p->next.load()) {
                            int b = p->hash & len;
                            if (b != runBit) {
                                runBit = b;
                        ...
                        delayDispose.ptr = [=, &keepPin]() -> void {
                            keepPin.retire<RecLinkedNode>(f, lastRun);
                        };
                        ...
                        for (Node* e = t->first; e != nullptr; e = e->next) {
                            int h = e->hash;
                            TreeNode* p = new TreeNode(h, e->key, e->val.load(),
                                                       nullptr, nullptr);
                            if ((h & len) == 0) {
                        ...
                        if (!flag_treebin_lc && !flag_treebin_hc) {
                            tree_bin_ptr = t;
                            rec_bytes +=
                                sizeof(TreeBin) + (lc + hc) * sizeof(TreeNode);
                        }
                        ...
                        delayDispose.ptr = [=, &keepPin]() -> void {
                            keepPin.retire<RecPartialTree>(tree_bin_ptr, lo_ptr,
                                                           hi_ptr);
                        };
                    }
                    advance = true;
    ...
    void tryPresize(int size, Pin& keepPin) {
        int c =
            (size >= (int)(static_cast<unsigned int>(MAXIMUM_CAPACITY) >> 1))
                ? MAXIMUM_CAPACITY
                : tableSizeFor(size + (static_cast<unsigned int>(size) >> 1) +
                               1);
        int sc;
        while ((sc = sizeCtl.load()) >= 0) {
            ...
            if (c <= sc || n >= MAXIMUM_CAPACITY)
                ...
                if (sizeCtl.compare_exchange_strong(
    ...
    void treeifyBin(BucketTable* localTable, int index, Pin& keepPin) {
        ...
        if (localTable != nullptr) {
            ...
            else if ((b = tabAt(localTable->tableArray, index)) != nullptr &&
                     b->hash >= 0) {
                std::lock_guard<std::mutex> control(
                    localTable->lock_levels[index]);
                if (tabAt(localTable->tableArray, index) == b) {
                    ...
                    for (Node* e = b; e != nullptr; e = e->next.load()) {
                        ...
                        TreeNode* p = new TreeNode(
                            e->hash, e->key, e->val.load(), nullptr, nullptr);
    ...
    void addCount(long x, int check, Pin& keepPin) {
        ...
        if (check >= 0) {
            ...
            BucketTable* nt;
            int n, sc;
            while (s >= (long)(sc = sizeCtl.load()) &&
                   (n = (localTable = table.load())->length) <
                       MAXIMUM_CAPACITY) {
                int rs = resizeStamp(n) << RESIZE_STAMP_SHIFT;

                if (sc < 0) {
                    if (sc == rs + MAX_RESIZERS || sc == rs + 1 ||
                        (nt = nextTable.load()) == nullptr ||
    ...
    BucketTable* helpTransfer(BucketTable* localTable, Node* f, Pin& keepPin) {
        ...
        while (nextTab == nextTable.load() && table.load() == localTable &&
               (sc = sizeCtl.load()) < 0) {
            if (sc == rs + MAX_RESIZERS || sc == rs + 1 ||
                transferIndex.load() <= 0)
                break;
...
public:
    ...
    private:
        ConstIterator(ConcurrentHashMap* map)
            : keepPin(map),
              table(map->table.load()),
              curr(tabAt(table->tableArray, 0)),
              index(0) {
        ...
        ConstIterator& operator++() {
            ...
            for (;;) {
                if ((++index) < table->length) {
                    ...
                    if (curr != nullptr) {
                        if (curr->hash == TREEBIN) {
                            curr = static_cast<TreeNode*>(
                                static_cast<TreeBin*>(curr)->first);
                        }
                        return *this;
                    } else {
                        continue;
    ...
    bool insertAbsent(const K& key, const V& value) {
        return insert(key, const_cast<V*>(&value), true);
    }
    ...
    bool eraseEqual(const K& key, const V& value) {
        return erase(key, const_cast<V*>(&value), true);
    }
            ...
            if (absent && fh == hash && KeyEqual()(f->key, key)) {
                ...
                if (f == tabAt(tab, i)) {
                    if (fh >= 0) {
                        ...
                        Node* p;
                        if ((p = tb->putTreeVal(hash, key, *value, keepPin)) !=
                            nullptr) {
                            if (!absent) {
                                auto old = p->val.load();
        ...
        for (;;) {
            ...
            if (f == tabAt(tab, i)) {
                if (fh >= 0) {
                    ...
                    // int value;
                    if ((r = t->root) != nullptr &&
                        (p = r->findTreeNode(hash, key)) != nullptr) {
                        oldVal = p->val.load();
                        if (!equal || (*oldVal) == (*value)) {
