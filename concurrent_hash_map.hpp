...
private:
  ...
  public:
    BucketTable(int n)
        : tableArray(new std::atomic<Node *>[n]()),
          lock_levels(new std::mutex[n]()), length(n), share() {}
  ...
  public:
    TreeNode(int hash, const K &key, const V &val, Node *next, TreeNode *parent)
        : Node(hash, key, val, next), parent(parent), left(nullptr),
          right(nullptr), prev(nullptr), red(false) {}
  ...
  public:
     ...
     */
    TreeBin(TreeNode *b)
        : Node(TREEBIN, K(), V()), root(nullptr), first(nullptr),
          waiter(nullptr), lockState(0) {
      this->shallow = false;
      this->first = b;
      TreeNode *r = nullptr;
      TreeNode *x = b;
        ...
        if (xp == (xppl = xpp->left)) {
          ...
          if (xppl != nullptr && xppl->red) {
            ...
            if (x == xp->left) {
              ...
              xpp = (xp = x->parent.load()) == nullptr ? nullptr
                                                       : xp->parent.load();
            }
            if (xp != nullptr) {
              xp->red = false;
    ...
    bool flagAndRecTreeNode(bool flag, TreeNode *node, Pin &keepPin,
  ...
  static bool casTabAt(std::atomic<Node *> *tab, int i, Node *&old,
                       Node *newNode) {
    ...
    for (int i = 0, bound = 0;;) {
      ...
      if (i < 0 || i >= len || i + len >= nextn) {
        ...
        if (sizeCtl.compare_exchange_strong(sc, sc - 1)) {
          ...
          i = len; // recheck and computes how many bytes should to be reclaimed
        }
        ...
        std::lock_guard<std::mutex> control(
            localTable->lock_levels[i]); // std::cout << "KKKK" << std::endl;
        if (tabAt(tab, i) == f) {
          ...
          if (fh >= 0) {
            ...
            for (Node *e = t->first; e != nullptr; e = e->next) {
              int h = e->hash;
              TreeNode *p =
                  new TreeNode(h, e->key, e->val.load(), nullptr, nullptr);
              if ((h & len) == 0) {
  ...
  void tryPresize(int size, Pin &keepPin) {
    ...
    while ((sc = sizeCtl.load()) >= 0) {
      ...
      if (c <= sc || n >= MAXIMUM_CAPACITY)
        ...
        if (sizeCtl.compare_exchange_strong(sc, (rs << RESIZE_STAMP_SHIFT) + 2))
      ...
      if ((n = localTable->length) < MIN_TREEIFY_CAPACITY)
        ...
        if (tabAt(localTable->tableArray, index) == b) {
          ...
          for (Node *e = b; e != nullptr; e = e->next.load()) {
            ++num;
            TreeNode *p =
                new TreeNode(e->hash, e->key, e->val.load(), nullptr, nullptr);
  ...
  void addCount(long x, int check, Pin &keepPin) {
    ...
    if (check >= 0) {
      ...
      while (s >= (long)(sc = sizeCtl.load()) &&
             (n = (localTable = table.load())->length) < MAXIMUM_CAPACITY) {
        int rs = resizeStamp(n) << RESIZE_STAMP_SHIFT;

        if (sc < 0) {
          if (sc == rs + MAX_RESIZERS || sc == rs + 1 ||
              (nt = nextTable.load()) == nullptr || transferIndex.load() <= 0)
            break;
          if (sizeCtl.compare_exchange_strong(sc, sc + 1)) {
  ...
  BucketTable *helpTransfer(BucketTable *localTable, Node *f, Pin &keepPin) {
    ...
    while (nextTab == nextTable.load() && table.load() == localTable &&
           (sc = sizeCtl.load()) < 0) {
      if (sc == rs + MAX_RESIZERS || sc == rs + 1 || transferIndex.load() <= 0)
        break;
      if (sizeCtl.compare_exchange_strong(sc, sc + 1)) {
...
public:
  ConcurrentHashMap()
      : ConcurrentBridge<ConcurrentHashMap<K, V>>(), table(nullptr),
        nextTable(nullptr), baseCount(0), sizeCtl(0), transferIndex(0) {
    initTable();
  }
  ...
  private:
    ConstIterator(ConcurrentHashMap *map)
        : keepPin(map), table(map->table.load()),
          curr(tabAt(table->tableArray, 0)), index(0) {
