...
template <typename T> class ms_queue : sebr::ConcurrentBridge<ms_queue<T>> {
  struct Node {
  ...
  class RecLockFreeNode : public sebr::ReclaimBridge {
    Node *node;
...
public:
  ms_queue()
      : sebr::ConcurrentBridge<ms_queue<T>>(), Head(new Node()),
        Tail(Head.load()) {}
  ...
  bool pop(T *ptr) {
    ...
    pin.retire<RecLockFreeNode>(head);
    return true;
  }
...
void test_scalable_queue(int count, int num) {
...

  {
    ...
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(
        endTime - beginTime);
    std::cout << "push/pop elapsed time is " << elapsedTime.count()
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < times; ++i) {
    std::thread thread(
        []() -> void { test_scalable_queue(n_const, nthreads_const); });
    thread.join();
  }
