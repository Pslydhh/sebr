...
template <typename T>
class RecSingleNode : public ReclaimBridge {
public:
    ...
    UNLINK_TAGGED_NODE:
        ...
        while (next != this->sentinel) {
            if (Next<T>::is_tagged(next)) {
                ...
                T* after_tagged_node =
                    Next<T>::untagged_address(next->next.load());
                while (Next<T>::is_tagged(after_tagged_node)) {
                    after_tagged_node = Next<T>::untagged_address(
                        after_tagged_node->next.load());
                }
        ...
        // while next == sentinel => There is no unlink threadHandle in the
...
public:
    ThreadHandle(ThreadHandle* sentinel,
...
public:
    ...
    ThreadHandle* bind() {
        ...
        ThreadHandle* handle = aggregate.get_thread_handle(
            this, &sentinel, &global_epoch, bytes_gc_threshold,
            bytes_epoch_threshold);
        return handle;
    }
